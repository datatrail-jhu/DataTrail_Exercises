...
You'll notice both datasets have pretty messy looking column names that are annoying to use. Use the `janitor::clean_names()` function to clean these up. Don't forget to reasign the data objects!

```{r}

```

Now take a look at the variable names again to see how they have changed.
...
Let's take a closer look at `life_expectancy_at_birth_females_males_years` as an example. Use the `head()` function and print out the first 10 rows to see.  

```{r}

```

Let's also take a closer look at `co2_emission_estimates_million_tons_tons_per_capita` as an example. Use the `count()` function to print out the unique values and how often they occur.  
...
Let's check to see if this did what we expected. We can use the count function to look at the unique values for each of these variables. We can pipe the output in a large print function to see all of the values.

```{r}
count(un_df, life_expectancy_at_birth_females_males_years) %>% print(n = 211) 
count(un_df, co2_emission_estimates_million_tons_tons_per_capita) %>% print(n = 211)
```
